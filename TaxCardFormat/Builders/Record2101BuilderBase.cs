...
public class Record2101BuilderBase(int lbnr, List<TaxRecord> records) : TaxFileBuilderBase(lbnr, records)
{
    ...
    {
        if (cpr.Length != 10) throw new ArgumentException("The CPR number should be 10 characters in length");
        Records.Add(new Record2101
        {
            Lb_nr = Lb_nr++,
            Rec_nr = 2101,
            AnsættelsesDato = AnsættelsesDato,
            FratrædelsesDato = fratrædelsesDato,
            GenRekvivering = genrevivering
                ? 'R'
                : ' ',
            personCpr = cpr,
